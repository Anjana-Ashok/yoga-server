...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@yoga-master.qkmnh.mongodb.net/?retryWrites=true&w=majority&appName=yoga-master`;
...
let classesCollection, cartCollection, userCollection, paymentCollection, enrolledCollection, appliedCollection; // Declare the collection variable here
...
async function run() {
  try {
    ...
    // Create a db and collections
    const database = client.db("yoga-master");

    userCollection = database.collection("users");
    classesCollection = database.collection("classes"); // Assign the collection here
...
app.post("/api/set-token", async (req, res) => {
  const user = req.body;
  var token = jwt.sign(user, process.env.ACCESS_SECRET, {
    expiresIn: '24h'
  });
  res.send({ token })
...
app.post('/new-class', verifyJWT, verifyInstructor, async (req, res) => {
  ...
  // Ensure the MongoDB client is connected before performing operations
  if (!classesCollection) {
    res.status(500).send("Collection not initialized");
    return;
  }
  try {
...
app.get('/admin-stats', verifyJWT, verifyAdmin, async (req, res) => {
  // get approved classes, pending classes and instructors
